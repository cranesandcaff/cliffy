...
export class CLI {
   ...
   * All commands
   */
  private readonly commands: { [command: string]: ICommand } = {};
  /**
   * Readline interface
   * *
   * @type {ReadLine}
  ...
  private async executeCommand(commandStr: string): Promise<any> {
    ...
    const action = command.command.action(params, options.options);
    const promIsPromise = action instanceof Promise;

    try {
      if (promIsPromise) return await action;
    } catch (error) {
      console.log(error);
    }
  }
   ...
   * Register a command
   * *
   * @param {string} [commandName] - Example: hello-world
   * @param {Command} [command]
   * *
   * @return {CLI}
   */
  public addCommand(commandName: string, command: ICommand): this {
  ...
  /**
   * Register a command
   * TODO: Remove there methode, use "addCommand"
   * *
   * @param {string} [commandName] - Example: hello-world
   * @param {Command} [command]
   * *
   ...
   * @return {CLI}
   */
  public command(commandName: string, command: ICommand): this {
    console.warn('"command" deprecated, use "addCommand"');
    return this.addCommand(commandName, command);
  }
   ...
   * Simple exit command
   * *
   * @param {string} [commandName] - Exit command (Default "exit")
   * *
   * @return {CLI}
   */
  public addExitCommand(commandName: string = 'exit'): this {
