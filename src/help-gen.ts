...
function genUsage(command: string, val: ICommand) {
...
export function printCommandHelp(command: string, val: ICommand) {
  log('');
  if (val.description) {
    log(val.description + '\n');
  }
...
function printOptions(val: ICommand) {
...
function printSubCommands(val: ICommand) {
  if (val.subcommands) {
    log('Sub-Commands:\n');
    const commands: any = {};
...
export function printOverviewHelp(commands: { [command: string]: ICommand }) {
